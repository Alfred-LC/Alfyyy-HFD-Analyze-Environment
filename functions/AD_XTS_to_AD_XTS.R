...
calAggreMeasures <- function(stockprices_xts, ExchangeCode, tradeType, lengthPeriod, unitPeriod) {
  ...
  intrameasures_xts <- period.apply(stockprices_xts, INDEX=period_lenthp,FUN=aggreFunLst1) %>% align.time(n=300)
...
ad_xts_rds_to_ad_aggtrademeasures_xts <- function(path, filename,lengthPeriod,unitPeriod) {
...
lapply(stock_xts.lst, function(stockprices_xts){
  ...
  intrameasures_xts <- calAggreMeasures(stockprices_xts,"all","all",lengthPeriod, unitPeriod)
  ...
  if(length(exchangeCodes) > 1){
    lapply(exchangeCodes, function(exchangeCode){
      intrameasures_anexchange.xts <- calAggreMeasures(stockprices_xts, exchangeCode, "all", lengthPeriod, unitPeriod)
      ...
      lapply(tradetypes, function(tradetype){
        intrameasures_atradetype.xts <- calAggreMeasures(stockprices_xts, exchangeCode, tradetype,lengthPeriod, unitPeriod)
    ...
    lapply(tradetypes, function(tradetype){
      intrameasures_atradetype.xts <- calAggreMeasures(stockprices_xts, "all", tradetype,lengthPeriod, unitPeriod)
      intrameasures_xts <<- merge(intrameasures_xts, intrameasures_atradetype.xts)
    })
